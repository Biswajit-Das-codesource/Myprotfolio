...
const ProjectCard = ({ title, description, tags, icon, link }) => {
  ...
  return (
    <a href={link} target="_blank" rel="noopener noreferrer">
      <div
        className="relative p-6 rounded-xl border border-gray-700 overflow-hidden transition-shadow duration-300 shadow-xs shadow-cyan-500/50 cursor-pointer min-h-[257px]"
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map((tag, index) => (
            <span
              key={index}
              className={`${
                !theme ? "bg-gray-200 text-black" : "text-white bg-gray-800"
...
const ProjectsSection = () => {
  ...
  return (
    <>
    <div className={`min-h-screen w-full ${!theme ? "bg-white" : "bg-zinc-950"} flex justify-center items-center flex-col`}>
      <section
